...
/* Code des verrous ""voies"" */ 
pthread_rwlock_t lock;

pthread_rwlock_t mutAB[Num_Stations][Num_Stations];
pthread_rwlock_t mutBA[Num_Stations][Num_Stations];
pthread_rwlock_t mutFifo;
...
{	
	...
	pthread_rwlock_rdlock(&mutFifo);   // file d'attente des trains des deux sens.  
	pthread_rwlock_rdlock(&mutAB[ville_depart][ville_arrivee]); // bloquer les autres trains du sens "AB" pour protéger l'incrémentation de leurs nb_train,verrouiller la section critique voie(V_dp,V_ar)
	nb_train = nb_train + 1;
	if(nb_train == 1) 
		pthread_rwlock_rdlock(&mutBA[ville_depart][ville_arrivee]);// bloquer le passage des trains du sens "BA" de rentrer à la voie(V_dp,V_ar),verrouiller la section critique voie(V_dp,V_ar)
	pthread_rwlock_unlock(&mutAB[ville_depart][ville_arrivee]);        // permet aux autres trains du sens "AB" de rentrer à la voie 
	pthread_rwlock_unlock(&mutFifo); 
	...
	/****************************************************/
	pthread_rwlock_rdlock(&mutAB[ville_depart][ville_arrivee]);//  bloquer les autres trains du sens "AB" pour modifier le nb_train  à l'interieur de la voie(V_dp,V_ar)
	nb_train = nb_train - 1;
		if(nb_train == 0)	
			pthread_rwlock_unlock(&mutBA[ville_depart][ville_arrivee]);
	pthread_rwlock_unlock(&mutAB[ville_depart][ville_arrivee]);
...
{	
	pthread_rwlock_rdlock(&mutFifo);  
	pthread_rwlock_rdlock(&mutBA[ville_depart][ville_arrivee]);
	nb_train = nb_train + 1;
	if(nb_train == 1) 
		pthread_rwlock_rdlock(&mutAB[ville_depart][ville_arrivee]); 
...
{	FILE * fichier;
...

    {
        ...
        fprintf(fichier,"\n Le temps consommé par le train ELBA est  :%lf S (Verrous)\n",(double)(t2 - t1)/(double) clk_clk); // Écriture dans le fichier
...
{	FILE * fichier;
...

    {
        ...
        fprintf(fichier,"\n Le temps consommé par le train ELAO est  :%lf S (Verrous)\n",(double)(t2 - t1)/(double) clk_clk); // Écriture dans le fichier
...
{	FILE *fichier;
...

    {
        ...
        fprintf(fichier,"\n Le temps consommé par le train DEBA est  :%lf S (Verrous)\n",(double)(t2 - t1)/(double) clk_clk); // Écriture dans le fichier
...
int main(int argc, char* argv[])
{
	...
	/* Creer autant de trains que
		necessaire */
	pthread_rwlock_init(&mutAB[Num_Stations][Num_Stations], NULL);
	pthread_rwlock_init(&mutBA[Num_Stations][Num_Stations], NULL);
	pthread_rwlock_init(&mutFifo, NULL);
	...
	pthread_rwlock_destroy(&mutFifo);
	pthread_rwlock_destroy(&mutAB[Num_Stations][Num_Stations]);
	pthread_rwlock_destroy(&mutBA[Num_Stations][Num_Stations]);
    return 0;        
